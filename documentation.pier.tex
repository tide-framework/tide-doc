...
use the pharo cli with the \sout{config option to load tide.
...
- In the World menu that appears, click on \ct{Workspace}
...
{[}{[}{[}
TDServer startOn: 5000. \symbol{34}Start the server on port 5000\symbol{34}
TDServer stop. \symbol{34}Stop any running server\symbol{34}
{]}{]}{]}
...
Note that not all \symbol{34}exposed\symbol{34} objects have to be subclasses of ==TDPresenter==. As we will
...
{[}{[}{[}
\{
  \symbol{34}\_\_id\_\_\symbol{34}:\symbol{34}bwv8m74bhgzmv0dgvzptuy4py\symbol{34},
  \symbol{34}actions\symbol{34}:\{
    \symbol{34}increase\symbol{34}:\symbol{34}$/$my-counter?\_callback=359446426\symbol{34},
    \symbol{34}decrease\symbol{34}:\symbol{34}$/$my-counter?\_callback=523483752\symbol{34}
  \},
  \symbol{34}state\symbol{34}:\{
    \symbol{34}count\symbol{34}:0
  \}
\}
{]}{]}{]}
...
- Presenter classes are registered as handlers, not instances. Tide will create \symbol{34}per session\symbol{34} instances of the registered class meaning that presenters are not shared between user sessions.
...
Interacting with proxies is performed via messages. However we have two kinds of messages. Messages sent to proxies will be resolved using their \symbol{34}\symbol{34}state\symbol{34}\symbol{34} and \symbol{34}\symbol{34}actions\symbol{34}\symbol{34} as defined on the server-side. SD: what is an actionplus? we did not introduce the terms like that.
...
{[}{[}{[}
\symbol{34}synchronous state call\symbol{34}
myProxy count. \symbol{34}=$>$ 0\symbol{34}
{]}{]}{]}
...
{[}{[}{[}
\symbol{34}async action call\symbol{34}
myProxy increase; then: {[}
    myProxy count \symbol{34}=$>$ 1\symbol{34} {]}
{]}{]}{]}
