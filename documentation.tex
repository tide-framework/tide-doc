...
\begin{itemize}
\item  Presenter classes are registered as handlers, not instances. Tide will create \symbol{34}per session\symbol{34}
\end{itemize}
...
\begin{itemize}
\item  The entry point will have a \ct{handler} associated with a fixed entry point 
\end{itemize}

  url \ct{'$/$my-counter'}. When someone will query that registered url, the presenter 
  will generate \ct{JSON} data corresponding to its state and actions, and the handler to 
