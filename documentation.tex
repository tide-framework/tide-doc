...
To get started with Tide, we will implement the traditional counter example as shown in the following figure \ref{tideCounter}. 
...
Messages sent to proxies will be resolved using their \textbf{state} and \textbf{actions+} as defined on 
the server-side.
...
\chapter{ Root presenters}\chapter{ Answering new presenters from action callbacks}\chapter{ Builtin presenter classes}\chapter{ Custom presenters}\chapter{ Presenters and security}\part{ Managing sessions}\part{ Handlers}
\begin{todo}
...
\chapter{ Creating file upload entry points}\part{ Handling exceptions}\part{ A more advanced example: }
